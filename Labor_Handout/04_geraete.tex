...
\begin{sidewaystable}[h!]
	\renewcommand*{\arraystretch}{1.2}
	\centering
	\caption{Übersicht von Pumpen}
	\label{tab:pumpen}
	%\resizebox{\textwidth}{!}{
		\begin{tabulary}{1.2\textwidth}{J|J|J|JJJJJJ}
			\hline
				\textbf{Pumpe} & \textbf{selbstansaugend?}&\textbf{Förderleistung}&\textbf{Max. Druck}&\textbf{Vakuum}&\textbf{Vorteil}&\textbf{Nachteil}&\textbf{Anwendung}&\textbf{Bemerkung}\\
				\hline
				Hubkolbenpumpe &Ja&verschiedene&&&&&&\\
				Kreiselpumpe/ Zentrifugalpumpe&Nein (und Ja)&Leistungsbreiche&&&&&&\\
				Flüssigkeitsringpumpe &Ja&möglich&&&&&&\\
				\hline
				Schlauchpumpe &&&&&&&&\\
				Drehschieberpumpe &&&&&&&&\\
				Membranpumpe &&&&&&&&\\
...
Die Membranpumpe ist eine Sonderform 

\subsection{Füllkörper}
\begin{itemize}
	\item zylindrische Ringe
	\item \textsc{Raschig}-Ringe
	\item \textsc{Pall}-Ringe
	\item Sattelkörper
\end{itemize}
